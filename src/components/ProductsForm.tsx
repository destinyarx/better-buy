...
import { UNIT_MEASUREMENT, DEFAULT_UNIT_MEASUREMENT, CONVERSION_TABLE, LABEL, UNIT_ICON } from '@/constants/units'
  ...
  const handleChange = (id: number, field: keyof Product, value: string) => {
    ...
    setRows((prev) =>
      prev.map((row) => {
        ...
        if (field === 'price' || field === 'quantity' || field === 'unitMeasurement') {
          const price = Number(updatedRow.price) || 0;
          const unitPrice = Number(updatedRow.quantity) * CONVERSION_TABLE[unit][updatedRow.unitMeasurement];
          updatedRow.basePrice = price / unitPrice;
        }
              ...
              return (
                <div key={row.id} className="px-6 py-6">
                  <div className="grid items-start gap-10 lg:grid-cols-[5%_auto_17%_22%_12%_5%]">
                    ...
                    <div className="flex justify-center text-[3.5rem] -mt-3">
